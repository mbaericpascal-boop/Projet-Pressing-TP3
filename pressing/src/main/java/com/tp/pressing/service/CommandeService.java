...
public class CommandeService {
    ...
    public Commande ajouterCommande(Commande commande) {
        ...
        if (sauvegardee.getEmailClient() != null && !sauvegardee.getEmailClient().isEmpty()) {
            try {
                ...
                System.err.println("⚠️ Erreur email confirmation : " + e.getMessage());
            }
        }
        return sauvegardee;
    }
    ...
    public Commande annulerCommande(Long id) {
        ...
        if (c.getStatut().equals("TERMINEE")) {
            throw new RuntimeException(
                "Impossible d'annuler la commande #" + id + " — elle est déjà terminée."
            );
        }
        if (c.getStatut().equals("ANNULEE")) {
            throw new RuntimeException("La commande #" + id + " est déjà annulée.");
        }
        c.setStatut("ANNULEE");
        c.setDateTerminaison(LocalDateTime.now());
        return commandeRepository.save(c);
